...
namespace _1512262_YOLO_GUI_BT01
{
    public partial class YOLO : Form
    {
        ...
        private bool checkOK(string program_path)
        {
            ...
            bool yolov3cfgExist = File.Exists(program_path + "yolov3.cfg");
            if (yolov3cfgExist == false)
            {
                MessageBox.Show("Missing file yolov3.cfg", "Alert!");
                return false;
            }
            bool yolov3weightsExist = File.Exists(program_path + "yolov3.weights"); ;
            if (yolov3weightsExist == false)
            {
                MessageBox.Show("Missing file yolov3.weights", "Alert!");
                return false;
            }
            bool cocodataExist = File.Exists(program_path + "\\cfg\\coco.data");
            if (cocodataExist == false)
            {
                MessageBox.Show("Missing file cfg/coco.data", "Alert!");
                return false;
            }
        ...
        string img_path = "";
        string program_path = "";
        string command = "darknet detector test cfg/coco.data yolov3.cfg yolov3.weights -dont_show ";
